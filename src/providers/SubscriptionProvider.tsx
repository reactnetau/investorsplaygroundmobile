...
function describeOfferingSetup(offerings: PurchasesOfferings | null) {
  ...
  if (!offerings || offeringIds.length === 0) {
    return 'No RevenueCat offerings were returned. Create an offering in RevenueCat, attach your product to a package, and mark the offering as Current.';
  }
  ...
  if (packageCount === 0) {
    return `RevenueCat returned offering${offeringIds.length === 1 ? '' : 's'} (${offeringIds.join(', ')}), but no packages. Add your product to a package in the offering.`;
  }
...
async function syncSubscriptionState(appUserId: string, customerInfo: CustomerInfo, entitlementId: string) {
  try {
    ...
    if (currentUser.userId !== appUserId || !session.tokens?.idToken) {
      return;
    }
  } catch {
    return;
  ...
  try {
    ...
    if (name === 'NoSignInUser' || message.includes('NoSignInUser')) return;
    throw err;
  }
}
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const [nextOfferings, nextCustomerInfo] = await Promise.all([
        withRevenueCatTimeout(Purchases.getOfferings(), 'RevenueCat offerings did not load. Check your product metadata and RevenueCat offering setup.'),
  ...
  const purchaseCurrentPackage = async () => {
    ...
    if (!nextPackage) {
      nextOfferings = await withRevenueCatTimeout(
        ...
        'RevenueCat offerings did not load. Check your product metadata and RevenueCat offering setup.'
      );
      setOfferings(nextOfferings);
    ...
    try {
      const result = await withRevenueCatTimeout(
        ...
        'The purchase did not finish. Close the purchase sheet if it is open, then try again.',
        120000
      );
  ...
  const restorePurchases = async () => {
    ...
    try {
      const nextCustomerInfo = await withRevenueCatTimeout(
        ...
        'Restore purchases did not finish. Check your account and try again.',
        120000
      );
      setCustomerInfo(nextCustomerInfo);
      setLocalSubscriptionActive(!!getActiveEntitlement(nextCustomerInfo, entitlementId)?.isActive);
